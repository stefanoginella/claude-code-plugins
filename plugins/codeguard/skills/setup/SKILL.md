...
description: >-
  ...
  far, to get guardrails in place before real coding begins. For a
  read-only report on what safeguards already exist and whether they
  still work, use the codeguard status skill instead.
---
...
- If a **Definition-of-Done Stop hook** was approved, wire it per
  `references/agent-guardrails.md`, starting from the shipped script
  template in `assets/dod-check.sh` (copy it to
  `.claude/hooks/dod-check.sh`, fill in the placeholders, `chmod +x`) —
  the reference covers the `settings.json` vs `settings.local.json`
  choice, the re-run rule (find codeguard's entry by its script path,
  merge rather than overwrite, never touch other hooks), making sure
  `.claude/` isn't gitignored while the `.claude/.dod-attempts` state
  file *is*, and how to verify the hook without tripping it on yourself
  mid-setup.
...
`.claude/settings.local.json`) along with its script and the
`.claude/.dod-attempts` state file, and drop the generated
hook/CI/config files — `git grep -l "generated by codeguard"` lists every
marker-bearing file, so removal doesn't rely on memory.

Finally, point at `/codeguard:status` — the read-only companion skill —
as the way to re-check the setup's health later (hooks still wired, CI
still present, nothing drifted) without re-running setup.
...
- `assets/dod-check.sh` — the DoD Stop-hook script template; copy and
  fill in its placeholders rather than writing the script from scratch.
